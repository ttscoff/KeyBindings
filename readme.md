...
<tr>
	<td style="text-align:center;">⌃⌘w</td>
	<td style="text-align:center;"></td>
	<td style="text-align:center;"><code>|Fence code (new) |
...
|||||
|Markdown link (l)||| |
|⌃⌘w |l |t |create a link for selected text, cursor between ()</code><a href="[cursor]">selected text</a><code>( links without selected text first, these can produce a mess using multiple clipboards make a text selection before you run them)|
|⌃⌘w |l |c |create a link for selected text, inserting clipboard as url</code>[[cursor]selected text](clipboard contents)<code>|
|||||
|⌃⌘w | |s |SearchLink Basic Link |
|Link as image (i)||| |
|⌃⌘w |i |t |same as lt, but with image syntax</code><img src="[cursor]" alt="selected text" /><code>|
|⌃⌘w |i |c |same as lc, but with image syntax</code><img src="clipboard" alt="selected text" /><code>|
|⌃⌘w |i |d |same as lc, but with image syntax and &quot;+&quot; for Droplr links</code><img src="clipboard+" alt="selected text" />`</td>
</tr>
<tr>
